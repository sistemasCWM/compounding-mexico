    ...
    def _compute_get_outgoing_detail(self):
        if self and self.picking_type_id.code == "outgoing":
            for rec in self:
                txt_message = ""
                if rec.partner_id:
                    txt_message += "Dear " +'*'+ str(rec.partner_id.name)+'*'+","+"%0A%0A"+"Here is the order " + \
                        '*'+rec.name+'*'+"%0A%0A"+"Following is your order details."+"%0A%0A"
                    if rec.move_ids_without_package:
                        for picking in rec.move_ids_without_package:
                            if picking.quantity_done > 0.00:
                                txt_message += "%0A"+'*'+picking.product_id.name+'*'+"%0A"+"*Required Qty:* " + \
                                    str(picking.product_uom_qty)+"%0A"+"*Delivery Qty:* "+str(
                                        picking.quantity_done)+"%0A"+"________________________"+"%0A"
                            else:
                                txt_message += "%0A"+'*'+picking.product_id.name+'*'+"%0A"+"*Required Qty:* " + \
                                    str(picking.product_uom_qty)+"%0A" + \
                                    "________________________"+"%0A"
                    if rec.company_id.stock_send_pdf_in_message:
                        base_url = self.env['ir.config_parameter'].sudo(
                        ).get_param('web.base.url')
                        stock_url = "%0A%0A *Click here to download Report :* " + \
                            base_url+rec.get_do_download_report_url()
                        self.write(
                    ...
                    txt_message += "Here is the order "+'*'+rec.name+'*' + \
                        "%0A%0A"+"Following is your order details."+"%0A%0A"
                    if rec.move_ids_without_package:
                        for picking in rec.move_ids_without_package:
                            if picking.quantity_done > 0.00:
                                txt_message += "%0A"+'*'+picking.product_id.name+'*'+"%0A"+"*Required Qty:* " + \
            ...
            for rec in self:
                ...
                if rec.partner_id:
                    ...
                    if rec.company_id.stock_send_pdf_in_message:
                        ...
                        stock_url = "%0A%0A *Click here to download Report :*" + \
                            base_url+rec.get_ship_download_report_url()
                        self.write(
