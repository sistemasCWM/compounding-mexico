...
class DownloadReport(http.Controller):
    ...
    def download_order(self, order_id, report_type=None, access_token=None, message=False, download=False, **kw):
        ...
        report_type = 'pdf'
        download = True
        pdf, _ = request.env.ref('sale.action_report_saleorder').with_user(SUPERUSER_ID)._render_qweb_pdf([order_id])
        
        pdfhttpheaders = [('Content-Type', 'application/pdf'), ('Content-Length', u'%s' % len(pdf)),]
        if report_type == 'pdf' and download:
            filename = "%s.pdf" % (
                re.sub('\W+', '-', order_sudo._get_report_base_filename()))
    ...
    def download_invoice(self, order_id, report_type=None, access_token=None, message=False, download=False, **kw):
        try:
            order_sudo = self._document_check_access(
                'account.move', order_id, access_token=access_token)
        except (AccessError, MissingError):
            return '<br/><br/><center><h1><b>Oops Invalid URL! Please check URL and try again!</b></h1></center>'
        report_type = 'pdf'
        download = True
        pdf, _ = request.env.ref('account.account_invoices').with_user(SUPERUSER_ID)._render_qweb_pdf([order_id])
    ...
    def download_purchase_order(self, order_id, report_type=None, access_token=None, message=False, download=False, **kw):
        ...
        report_type = 'pdf'
        download = True
        pdf, _ = request.env.ref('purchase.action_report_purchase_order').with_user(SUPERUSER_ID)._render_qweb_pdf([order_id])
    ...
    def download_do(self, order_id, report_type=None, access_token=None, message=False, download=False, **kw):
        ...
        report_type = 'pdf'
        download = True
        pdf, _ = request.env.ref('stock.action_report_delivery').with_user(SUPERUSER_ID)._render_qweb_pdf([order_id])
    ...
    def download_shipment(self, order_id, report_type=None, access_token=None, message=False, download=False, **kw):
        ...
        report_type = 'pdf'
        download = True
        pdf, _ = request.env.ref('stock.action_report_picking').with_user(SUPERUSER_ID)._render_qweb_pdf([order_id])
    ...
    def download_payment(self, order_id, report_type=None, access_token=None, message=False, download=False, **kw):
        try:
            order_sudo = self._document_check_access(
                'account.payment', order_id, access_token=access_token)
        except (AccessError, MissingError):
            return '<br/><br/><center><h1><b>Oops Invalid URL! Please check URL and try again!</b></h1></center>'
        report_type = 'pdf'
        download = True
        pdf, _ = request.env.ref('account.action_report_payment_receipt').with_user(SUPERUSER_ID)._render_qweb_pdf([order_id])
